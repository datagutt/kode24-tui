export const translations: Record<string, string> = {
  ...
  navJobs:
    "Navigasjon: ↑↓ Bla gjennom jobber | Enter Se detaljer | s Søk | f Filtrer",
  ...
  paywallOrFormatting:
    "Dette kan skyldes betalingsmur eller formateringsproblemer",
...
export const t = (key: string, params?: Record<string, any>): string => {
  ...
  if (params) {
    trans = trans.replace(
      /{(\w+)}/g,
      (match, p) => params[p]?.toString() || match
    );
  }
  return trans;
};
